...
interface Technician {
    ...
    commission_type?: 'percent' | 'fixed'; // Opcional pois pode vir null de registros antigos
    is_active: boolean;
}
...
export function TeamPage() {
    ...
    async function loadTechnicians() {
        try {
            setIsLoading(true);
            setError(null);
            console.log('Carregando técnicos para empresa:', activeCompanyId);
    ...
    return (
        <div className="min-h-screen pb-20 p-4 md:p-8">
            ...
            {isLoading ? (
                ...
                <div className="flex flex-col items-center justify-center py-20 bg-slate-900/50 border border-white/5 rounded-3xl border-dashed">
                    <div className="w-16 h-16 bg-slate-800 rounded-full flex items-center justify-center mb-4 text-slate-500">
                        <UserPlus size={32} />
                    </div>
                    <h3 className="text-xl font-bold text-white mb-2">Nenhum técnico encontrado</h3>
                    <p className="text-slate-400 text-center max-w-md mb-6">
                        Você ainda não cadastrou ninguém na sua equipe. Adicione o primeiro técnico para começar.
                    </p>
                    <button
                        onClick={() => setIsFormOpen(true)}
                                        ...
                                        <div>
                                            ...
                                            <div className="flex items-center gap-2">
                                                <span className="text-xs px-2 py-0.5 rounded bg-slate-800 text-slate-400 border border-white/5">
                                                    {tech.commission_type === 'fixed' ? 'R$' : ''} {tech.commission_rate}{tech.commission_type === 'percent' || !tech.commission_type ? '%' : ''} Comissão
                                                </span>
                                            </div>
                                        </div>
                                    </div>
